...
export default function LocationBar() {
  ...
  return (
    <div
      className={`fixed inset-x-0 top-0 z-40 transform border-b border-cream/15 bg-cream/[0.35] shadow-[0_10px_30px_-14px_rgba(0,0,0,0.6)] backdrop-blur-xl transition-transform duration-500 ease-out-expo ${
      ...
      <div className="mx-auto flex max-w-shell items-center justify-between gap-3 px-5 py-2.5 sm:px-8">
        ...
        <div className="relative" ref={pickerRef}>
          ...
          >
            ...
            <span>
              {picked && <span className="text-cream/60">Johnny&rsquo;s </span>}
              {current}
            </span>
            <svg
              width="11"
                  ...
                  <li key={`${loc.city}-${loc.state}`}>
                    <button
                      ...
                      className={`flex w-full items-center justify-between px-4 py-2 text-left text-sm transition-colors hover:bg-cream/5 ${
                        active ? "font-medium text-cream" : "text-dim-cream"
                      }`}
                    >
                      <span>{loc.city}</span>
        ...
        {picked ? (
          ...
          <button
            type="button"
            disabled
            className="btn btn-sm inline-flex cursor-not-allowed items-center gap-2 border border-cream/25 bg-transparent text-cream/40"
          >
            Go to this location
            <svg
              width="14"
